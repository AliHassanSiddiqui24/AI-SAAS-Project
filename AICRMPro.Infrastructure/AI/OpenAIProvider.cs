...
public class OpenAIProvider : IAIProvider
{
    ...
    public async Task<AIProviderResponse> GenerateAsync(AIProviderRequest request)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                {
                    throw new InvalidOperationException("OpenAI API rate limit exceeded. Please try again later.");
                }
        ...
        catch (HttpRequestException ex)
        {
            ...
            {
                IsSuccess = false,
                ErrorMessage = $"OpenAI HTTP request failed: {ex.Message}",
                LatencyMs = (int)stopwatch.ElapsedMilliseconds
            };
        }
        ...
        catch (Exception ex)
        {
            ...
            {
                IsSuccess = false,
                ErrorMessage = $"OpenAI provider error: {ex.Message}",
                LatencyMs = (int)stopwatch.ElapsedMilliseconds
            };
        }
    }
    ...
    public async IAsyncEnumerable<string> GenerateStreamAsync(AIProviderRequest request)
    {
        ...
        {
            yield return $"OpenAI streaming error: {ex.Message}";
        }
    }
}
